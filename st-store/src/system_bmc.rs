...
impl SystemBmcTrait for DbSystemBmc {
    ...
    async fn upsert_waypoint(&self, _ctx: &Ctx, waypoint: Waypoint) -> Result<()> {
        db::upsert_waypoints(self.mm.pool(), vec![waypoint], Utc::now()).await
    }

    async fn get_num_systems(&self, _ctx: &Ctx) -> Result<i64> {
        let row = sqlx::query!(
            r#"
select count(*) as count
  from systems
    ...
    async fn get_num_waypoints(&self, _ctx: &Ctx) -> Result<i64> {
        let row = sqlx::query!(
            r#"
select count(*) as count
  from waypoints
    ...
    async fn get_waypoint(&self, _ctx: &Ctx, waypoint_symbol: &WaypointSymbol) -> Result<Waypoint> {
